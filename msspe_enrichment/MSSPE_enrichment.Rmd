...
alltarget_boxplot <- ggplot(only_viral_targets, aes(x = name_group, y = rpm_enrichment)) +
  ...
  scale_y_log10() +
  geom_jitter(width = 0.1, alpha = 0.6, size = 2.5, color = "#1565C0", stroke = 0.5) +  
  #dotted mean line
  geom_segment(
    data = mean_df,
  ...
  geom_text(data = mean_df, aes(x = name_group, y = mean_value, label = paste0("μ ", round(mean_value, 1))), vjust = -1, color = "black", size = 4, fontface = "italic") +
  scale_x_discrete() +  
  labs(
    title = "",
    x = "",
  ...
  theme(
    ...
    panel.border = element_rect(linewidth = .7, color = "black")
  )
...
viral_target_table <- viral_target_enrichment_summary %>%
  select(name_group, rpm_mngs, rpm_msspe, median_enrichment, mean_enrichment, iqr, p_value, p_adj, effect_r) %>% 
  rename(
    "Virus" = name_group,
    "rPM (mNGS)" = rpm_mngs,
    "rPM (MSSPE)" = rpm_msspe,
...
wilcox_summary_kbl = wilcox_summary %>%
  ...
  select(virus, mean_ngs, mean_msspe, percent_enrichment, iqr, p_value, p_adj, effect_r) %>% 
  rename(
    "Virus" = virus,
...
all_target_cov_plot = ggplot(paired_tbl, aes(x = virus, y = enrichment_breadth)) +
  ...
  geom_jitter(width = 0.1, alpha = 0.6, size = 2.5, color = "#7B1FA2") +  
  ...
  geom_text(data = mean_cov_df, aes(x = virus, y = mean_value, label = paste0("μ ", round(mean_value, 1))), vjust = -1, color = "black", size = 4, fontface = "italic") +
  labs(
    x = "",
...
# effect size for enrichment
wilcox_effsize(
  data = overall_tbl_summary,
  rpm_enrichment ~ 1,
  mu = 1
)
...
log2fc_plot = ggplot(overall_tbl_summary, aes(x = "All paired target detections (n=103)", y = rpm_enrichment)) +
  ...
  scale_y_log10() +
  geom_jitter(width = 0.1, alpha = 0.6, size = 2, color = "#1565C0") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
...
#effect size for coverage
wilcox_effsize(
  data = paired_tbl %>% 
    pivot_longer(cols = c(breadth_1x_NGS, breadth_1x_MSSPE), 
                 names_to = "method", 
                 values_to = "breadth"),
  breadth ~ method,
  paired = TRUE,
  ref.group = "breadth_1x_NGS"
)
...
cov_plot = ggplot(paired_tbl, aes(x = "All paired target detections (n=103)", y = enrichment_breadth)) +
  geom_boxplot(width = 0.4, fill = "gray90", color = "black", outlier.shape = 21, outlier.fill = "white") +
  geom_jitter(width = 0.1, alpha = 0.6, size = 2, color = "#7B1FA2") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
...
odds_ratio_table <- results_libprep %>%
  select(name_group, b, c, odds_ratio, ci_lower, ci_upper) %>% 
  rename(
    "Virus" = name_group,
    "MSSPE detections" = b,
    "mNGS detections" = c,
    "Odds Ratio (MSSPE/mNGS)" = odds_ratio,
...
if (exists("odds_ratio_table")) {
  export_table_png(
    latex_table = odds_ratio_table,
    table_dir = table_dir,
    output_basename = "odds_ratio_table"
  )
}
```
