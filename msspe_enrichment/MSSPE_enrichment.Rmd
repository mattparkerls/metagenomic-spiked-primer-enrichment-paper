...
# Extract HPV data from experiments 1 & 2 before filtering
hpv_data = merged_data %>% 
  filter(experiment == 1 | experiment == 2) %>% 
  filter(str_detect(name, regex("Alphapapillomavirus", ignore_case = TRUE)))

# Extract mNGS kobuvirus data from experiment 1 to compare to experiment 3
#pkv_data = merged_data %>% 
  #filter(experiment == 1) %>% 
  #filter(str_detect(name, regex("Aichivirus", ignore_case = TRUE)))

# Remove MSSPE libraries from experiment 1 (after extracting HPV data above)
# Experiment 1 MSSPE libraries are excluded from main analysis
...
```{r}
# Define viral targets to filter to the genus level
genus_targets = c(
  "Bocaparvovirus",
  "Sapelovirus",
  "Circovirus",
