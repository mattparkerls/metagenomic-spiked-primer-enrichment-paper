...
viral_target_enrichment_summary = only_viral_targets %>% 
  group_by(name_group) %>% 
  summarise(
    rpm_ngs = round(mean(NGS, na.rm = TRUE),2), # mean
...
wilcox_summary %>%
  ...
  kable(
    ...
    booktabs = TRUE,
    caption = "Genome coverage increase across all paired samples per viral target",
    digits = 3,
    align = "lrrr",   # adjust as needed: text left, numbers right
    format.args = list(big.mark = ",")
  ) %>%
...
if (exists("alltarget_boxplot_exp1")) {
  export_plot_png(alltarget_boxplot_exp1, fig_dir, "alltarget_boxplot_experiment1.png", width = 9, height = 5)
}
...
if (exists("coverage_enrichment_plot")) {
  export_plot_png(coverage_enrichment_plot, fig_dir, "coverage_enrichment_by_virus.png", width = 9, height = 5)
}
...
# Optional: export a LaTeX-styled table to PNG if available
