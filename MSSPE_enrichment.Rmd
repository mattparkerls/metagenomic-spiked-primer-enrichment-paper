...
sample_metadata = seq_metrics %>% 
  ...
  base_sample = tolower(paste0(base)),
...
viral_target_enrichment_summary = only_viral_targets %>% 
  ...
  summarise(
    ...
    fold_enrichment = round(mean(mean_rpm_enrichment, na.rm = TRUE),2),
    iqr = paste0(
...
overall_tbl_summary = overall_tbl %>% 
  group_by(name_group, library_prep) %>% 
  summarise(mean_rpm = mean(mean_nt_rpm))

#overall_tbl_summary %>% filter(library_prep == "MSSPE") %>% pull(mean_rpm) %>% mean()
...
ggplot(overall_tbl, aes(x = library_prep, y = mean_nt_rpm, fill = library_prep)) +
  ...
  labs(
    x = "",
    y = "mean rPM",
    title = "rPM Enrichment Across All Targeted Viruses"
  ) +
...
wilcox_summary = paired_tbl %>% 
  ...
  summarise(
    ...
    percent_enrichment = round(mean(breadth_1x_MSSPE - breadth_1x_NGS, na.rm = TRUE),2),
    iqr = paste0(
