...
csv_files <- list.files(path = "/Users/mattparker/Dropbox/Workspace/moops/manuscript1/data/experiment_3/sample_taxon_reports", pattern = ".*.csv$", full.names = TRUE)
...
sample_metadata = seq_metrics %>% 
  ...
  # Base sample: extract first alphanumeric block (S2, S3, PC1, etc.)
  base_sample = str_extract(core, "^[A-Za-z]+\\d+"),

  # Experiment classification logic
  experiment = case_when(
      str_detect(core, "C\\d+.*LP\\d+") ~ 3,  # Cxxx_LPx present
      str_detect(core, "_3$") ~ 2,           # only suffix _3 is exp2
      TRUE ~ 1
  )
) %>%
select(sample_name, base_sample, library_prep, experiment)
```
...
```{r}
# Detection summary
det_cols <- grep("^detect_", names(res2), value = TRUE)
det_summary <- res2 %>%
  group_by(depth) %>%
